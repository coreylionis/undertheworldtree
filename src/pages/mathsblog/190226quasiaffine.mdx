...
The important idea is to show that $$\mathcal{O}(\mathbb{A}^2 \setminus \{(0,0)\}) = k[x,y]$$;  then $$\mathbb{A}^2$$ has the same ring of regular functions as $$\mathbb{A}^2 \setminus \{(0,0)\}$$. We know that when $$X$$, $$Y$$ are varieties and $$Y$$ is affine we have an isomorphism $$\text{Hom}(X, Y) \cong \text{Hom}(A(Y), \mathcal{O}(X))$$, so that (by Yoneda lemma or otherwise) when  $$\mathcal{O}(Y) \cong A(Y) \cong k[x,y]$$ one has $$Y \cong \mathbb{A}^2$$, contradicting affineness of $$\mathbb{A}^2 \setminus \{(0,0)\}$$ if we show also that $$\mathbb{A}^2 \setminus \{(0,0)\} \not \cong \mathbb{A}^2$$. This step is subtle: Wiegand shows in [Wie78](https://doi.org/10.1112%2Fjlms%2Fs2-18.1.28) that all nonempty open subsets of $$\mathbb{A}^2$$ are homemorphic in positive characteristic, so the difference between the two spaces should be algebraic. 
...
$$\underline{\mathbb{A}^2 \setminus \{(0,0)\}) \text{ is not isomorphic to } \mathbb{A}^2}$$: Since a morphism of varieties induces a map of the rings of regular functions, an isomorphism $$f: \mathbb{A}^2 \xrightarrow{\sim} \mathbb{A}^2 \setminus \{(0,0)\}$$ would correspond to an automorphism of $$k[x,y]$$. Let $$f_{*}: k[x,y] \to k[x,y], g \mapsto g \circ f$$ be the associated isomorphism of rings (this is the map we get if we think of $$k[x,y]$$ as regular functions on each space), and notice that $$f$$ induces an isomorphism $$k \cong k[x,y]/(x,y) \to k[x, y]/(x \circ f, y \circ f)$$. It follows that $$(x\circ f, y \circ f)$$ is a maximal ideal, $$(x\circ f, y\circ f) = (x-a, y-b)$$ for some $$a, b \in k$$. 
